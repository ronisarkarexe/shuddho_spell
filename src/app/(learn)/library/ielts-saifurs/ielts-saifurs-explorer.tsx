...
import {
  IELTS_SAIFURS_PAGE_SIZE,
  ieltsSaifursPageSchema,
  type IeltsSaifursAccent,
  type IeltsSaifursEntryView,
  type IeltsSaifursMode,
  type IeltsSaifursPage,
} from '@/components/learning/ielts-saifurs-contracts';
import { apiFetch } from '@/lib/api/client';
import { useSpeech } from '@/lib/audio/use-speech';
import { DICTATION_RATE, SENTENCE_RATE } from '@/lib/audio/voices';
import { cn } from '@/lib/cn';

export interface IIeltsSaifursExplorerProps {
  readonly initialPage: IeltsSaifursPage;
  readonly initialAccent: IeltsSaifursAccent;
}
...
const ACCENT_LANG: Readonly<Record<IeltsSaifursAccent, string>> = {
  british: 'en-GB',
  american: 'en-US',
};

/**
 * IELTS Saifur's vocabulary: read the book, or learn the page, twenty-five
 * at a time. A separate shelf from the admission Saifur's list.
 */
export function IeltsSaifursExplorer({
  initialPage,
  initialAccent,
}: IIeltsSaifursExplorerProps): ReactElement {
  const [page, setPage] = useState<IeltsSaifursPage>(initialPage);
  ...
  const [mode, setMode] = useState<IeltsSaifursMode>('read');
  const [accent, setAccent] = useState<IeltsSaifursAccent>(initialAccent);
  ...
  useEffect(() => {
    ...
    void apiFetch('/api/v1/library/ielts-saifurs', {
      schema: ieltsSaifursPageSchema,
      query: {
        pageSize: IELTS_SAIFURS_PAGE_SIZE,
        page: pageNumber,
...
function ModeToggle({
  ...
  readonly mode: IeltsSaifursMode;
  readonly onChange: (mode: IeltsSaifursMode) => void;
}): ReactElement {
  return (
    <div
...
function AccentToggle({
  ...
  readonly accent: IeltsSaifursAccent;
  readonly onChange: (accent: IeltsSaifursAccent) => void;
}): ReactElement {
  return (
    <div
      aria-label="Accent"
...
function LetterStrip({
  ...
  readonly letters: IeltsSaifursPage['letters'];
...
function WordCard({
  ...
  readonly entry: IeltsSaifursEntryView;
  readonly accent: IeltsSaifursAccent;
}): ReactElement {
  return (
...
function MeaningBlock({ entry }: { readonly entry: IeltsSaifursEntryView }): ReactElement {
  return (
